...
class Transformer(nn.Module):
    def __init__(self, args, tokenizer):
        ...
        self.mask_task = AuxRecoveryMask(args, d_model=args.d_model, vocab_size=self.vocab_size)
    ...
    def forward(self, enc_inputs, ss_inputs):
        enc_outputs, enc_self_attns = self.encoder(enc_inputs)
        cls_pooling = self.pooling(enc_outputs[:, :1, :])
        poolig_logits = self.linear(cls_pooling.squeeze(1))

        if self.args.self_supervision == 'True':
            mask_logits = self.mask_task(enc_outputs, ss_inputs)
        else:
            mask_logits = None

        return poolig_logits, mask_logits